...
The Train menu enables automatic best-triplet selection by default. It holds
out GLDv2 landmark IDs that do not occur in the training split, evaluates the
same triplets after every epoch, saves the best epoch to the requested
checkpoint path, and saves the final epoch with `_last` appended. The selected
best checkpoint becomes the default in later prediction and DB menus.
